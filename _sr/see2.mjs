import { launch } from './h.mjs';
import { writeFileSync } from 'node:fs';
import { decode } from './png.mjs';

const b = await launch({ headful: process.argv.includes('--headful') });
const sleep = ms => new Promise(r => setTimeout(r, ms));

async function bbox(tag) {
  const full = await b.send('Page.captureScreenshot', { format: 'png' });
  writeFileSync(`_sr/${tag}.png`, Buffer.from(full.data, 'base64'));
  const { w, h, bpp, px } = decode(`_sr/${tag}.png`);
  let x0 = w, y0 = h, x1 = 0, y1 = 0, n = 0;
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
    const i = (y * w + x) * bpp;
    if (px[i] + px[i + 1] + px[i + 2] > 90) { n++; if (x < x0) x0 = x; if (y < y0) y0 = y; if (x > x1) x1 = x; if (y > y1) y1 = y; }
  }
  const geo = await b.evaluate(`JSON.stringify((() => { const c = document.querySelector('canvas');
    const r = c.getBoundingClientRect();
    return { rect: [r.width|0, r.height|0], attr: [c.width, c.height],
      iw: innerWidth, ih: innerHeight, dpr: devicePixelRatio }; })())`);
  console.log('%s: lit=%d bbox=%s canvas=%s', tag, n, n ? `${x0},${y0}-${x1},${y1}` : '-', geo);
}

try {
  await b.goto('/games/soccer-random/index.html');
  await sleep(12000);
  await bbox('a-boot');
  await b.evaluate(`window.dispatchEvent(new Event('resize')); 'ok'`);
  await sleep(2000);
  await bbox('b-resize');
  await b.evaluate(`document.querySelector('canvas').click(); 'ok'`);
  await sleep(3000);
  await bbox('c-click');
} finally {
  await b.close();
}
