...
function initPokiSdk(){/* The obfuscated IIFE that stood here was Poki's sitelock. It read
  window.location.hostname, allowed "localhost" and any ".poki-gdn.com" host, and
  otherwise navigated this page — and the top frame with it — to
  https://poki.com/sitelock, which serves Poki's "game not available" page. Served
  from 127.0.0.1 that check could only ever do one thing, so it is gone; the SDK
  call below is what this function was always for. See poki-local.js. */
...
function pokiSendLevelData(){/* refused: this sent a beacon to leveldata.poki.io on every level completion. Nothing here talks to that host. */}
