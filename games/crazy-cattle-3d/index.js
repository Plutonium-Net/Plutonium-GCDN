  ...
  return (
    function(moduleArg = {}) {
      ...
      var GodotFS = {
        ...
        _syncing: false,
        _mount_points: [],
        /* The engine asks this before it decides whether to sync at all, and
           answer 0 means it never persists anything — so this is the gate that
           has to move first, before either of the other two sites matters.
           See STORAGE.md section 10. */
        is_persistent: function() {
          return GodotFS._idbfs || (self.PluStore && self.PluStore.flush) ? 1 : 0
        },
        init: function(persistentPaths) {
          ...
          /* user:// is mounted in memory and seeded from the PluStore document
             instead of from IndexedDB (IDBFS), which is where this export used
             to keep it. With PluStore present there is nothing left to populate,
             so init resolves immediately; the IDBFS branch stays for a page that
             loads this engine without PluStore. */
          GodotFS._mount_points.forEach(function(path) {
            createRecursive(path);
            if (self.PluStore && self.PluStore.boot) {
              FS.mount(MEMFS, {}, path);
              self.PluStore.boot(FS, path)
            } else {
              FS.mount(IDBFS, {}, path)
            }
          });
        ...
        sync: function() {
          ...
          /* A flush rewrites the whole document out of the mounted tree, the
             same way the Unity player does — see STORAGE.md section 10. */
          GodotFS._syncing = true;
          if (self.PluStore && self.PluStore.flush) {
            GodotFS._mount_points.forEach(function(path) {
              self.PluStore.flush(FS, path)
            });
            GodotFS._syncing = false;
