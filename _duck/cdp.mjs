...
class Page {
  ...
  async navigate(url) {
    ...
    for (;;) {
      ...
      if (Date.now() - t0 > 60000) throw new Error('navigation never completed: ' + url);
      await sleep(150);
    }
  }
  ...
  key(type, code, key, vk) {
    return this.send('Input.dispatchKeyEvent', { type, code, key, windowsVirtualKeyCode: vk, nativeVirtualKeyCode: vk });
  }
  async screenshot(path) {
    const { data } = await this.send('Page.captureScreenshot', { format: 'png' }, 120000);
...
export async function launch({ port = 9336, width = 900, height = 700 } = {}) {
  const dir = mkdtempSync(join(tmpdir(), 'plu-duck-'));
  ...
  const events = { console: [], exceptions: [], requests: [], responses: [], failed: [] };
