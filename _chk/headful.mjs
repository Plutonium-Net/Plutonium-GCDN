...
const proc = spawn(CHROME, ['--disable-gpu', '--no-first-run', '--mute-audio', '--hide-scrollbars',
  '--remote-debugging-port=9341', '--user-data-dir=' + profile, '--window-size=1280,800', '--window-position=-32000,-32000', '--audio-output-channels=2', 'about:blank'], { stdio: 'ignore' });
let target = null;
for (let i = 0; i < 60 && !target; i++) {
  try { target = (await (await fetch('http://127.0.0.1:9341/json/list')).json()).find((t) => t.type === 'page'); } catch (e) {}
...
/* Second visit in the same profile: the game's own save is in the store this
   time, which is the state the user's browser had. */
ev.console.length = 0; ev.exceptions.length = 0;
console.log('\n--- old guard, second visit (save present)');
await send('Page.navigate', { url: origin + '/games/funny-shooter-2/index-old.html' });
for (let i = 0; i < 4; i++) { await sleep(5000); console.log('old guard 2nd ' + (5 * (i + 1)) + 's: ' + await raw(state)); }
console.log('old-guard 2nd-visit refusals: ' + JSON.stringify(ev.console.filter((c) => /refusing/.test(c)).map((c) => c.slice(0, 150))));
